...
parseBlossom :: BParser Program
parseBlossom = re defPrg
    where go prg = parseTop >>= insert prg . unwrap >>= re
...
parseTop = withPos $ tryList [over, dataDec, tlet, insDec, classDec]
